...
Ts = 0.02;
N  = 3000;
amp = 500;  % steps/s - the effective gain here is ~4x smaller than the bare
            % motor (belt reduces turntable deg/step), so if the identified
            % signal looks too small/noisy relative to encoder quantization,
            % increase this and re-run.
STEPS_PER_REV = 200 * 16;   % motor microsteps per MOTOR revolution
GEAR_RATIO = 4;              % motor:turntable
...
%unwrap position (y is TURNTABLE degrees - encoder is on the disc)
y_unwrapped = unwrap(y * deg2rad) / deg2rad;
y_unwrapped = y_unwrapped - y_unwrapped(1);

%%VELOCITY-DOMAIN GAIN CHECK
% Same check as the bare-motor system, now against the turntable-frame
% theoretical gain: 360 deg / (steps-per-motor-rev * gear ratio).
v = smoothdata(gradient(y_unwrapped, Ts), 'movmean', 5);
K_hat = u \ v;
fitPercent_vel = 100 * (1 - norm(v - K_hat*u) / norm(v - mean(v)));

fprintf('\nVELOCITY-DOMAIN GAIN CHECK\n');
fprintf('Theoretical gain:      %.5f deg/step\n', 360 / (STEPS_PER_REV * GEAR_RATIO));
fprintf('Velocity-domain gain:  %.5f deg/step (static-gain fit %.2f%%)\n', K_hat, fitPercent_vel);
...
title(sprintf('Velocity-Domain Gain Check (K = %.5f deg/step, fit %.2f%%)', K_hat, fitPercent_vel));
grid on;
...
% Unlike the bare motor (which was cleanly a single near-integrator
% pole), a belt is a compliant coupling between motor and turntable
% inertia - don't assume sys1 wins here. If sys3/sys4 (complex poles)
% fit meaningfully better than sys1, that's the signature of a real
% belt resonance, not overfitting, and should be kept rather than
% simplified away.
sys1 = tfest(data, 1, 0);
sys2 = tfest(data, 2, 0);
sys3 = tfest(data, 2, 1);
...
title('Turntable Position Plant Validation');
...
fprintf('\nIDENTIFIED TURNTABLE POSITION PLANT TRANSFER FUNCTION\n');
sys_p
...
sgtitle('Open-Loop Turntable Plant Response: Continuous vs Discrete');
...
%%EXPORT PLANT + CONTROLLER FOR HARDWARE TUNING
save(fullfile(scriptDir, 'plant_model_turntable.mat'), 'sys_p', 'C_pid');

fprintf('\nSystem ID Complete! Plots saved to %s\n', resultsDir);
fprintf('Plant + controller saved to %s\n', fullfile(scriptDir, 'plant_model_turntable.mat'));
