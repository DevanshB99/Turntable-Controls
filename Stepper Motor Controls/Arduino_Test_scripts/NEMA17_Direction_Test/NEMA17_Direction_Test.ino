/*
 * NEMA17 Direction Test
 * Determine which DIR pin state (HIGH/LOW) increases vs decreases
 * the encoder angle. Bare motor, no gearing.
 */
...
const float MAX_STEP_RATE = 5000;  // steps/s - matches Nema17_TMC_Combined.ino's clamp
...
void printHelp() {
  ...
  Serial.println("? - help");
  Serial.println("\nStart: e, then f (or b), then r ... let it run a bit ... s");
  Serial.println("Watch the motor while it runs and note CW/CCW yourself,");
