...
const float MAX_STEP_RATE = 3000;  // steps/s output clamp
...
// RootLocus_PI_Design_TMC.m's pid(Kp,Ki,Kd,Tf) so the linear model
// matches what's actually running here.
...
void homeToZero() {
  ...
  digitalWrite(DIR_PIN, error >= 0 ? LOW : HIGH);  // LOW increases encoder angle (measured)
...
void loop() {
  ...
  if (targetStepRate != 0) {
    ...
    if (now - lastStepTime >= stepInterval) {
      digitalWrite(DIR_PIN, targetStepRate >= 0 ? LOW : HIGH);  // LOW increases encoder angle (measured)
      digitalWrite(STEP_PIN, HIGH);
      delayMicroseconds(5);
      digitalWrite(STEP_PIN, LOW);
      lastStepTime = now;
