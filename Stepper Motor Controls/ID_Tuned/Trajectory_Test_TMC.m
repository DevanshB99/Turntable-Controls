...
%% PID GAINS - same convention as Hardware_PID_Tuning.m
if exist('C', 'var')
    Kp = C.Kp; Ki = C.Ki; Kd = C.Kd;
    Kp_final = Kp; Ki_final = Ki; Kd_final = Kd;
    save(fullfile(scriptDir, 'pid_rootlocus_TMC.mat'), 'Kp_final', 'Ki_final', 'Kd_final', '-append');
else
    load(fullfile(scriptDir, 'pid_rootlocus_TMC.mat'), 'Kp_final', 'Ki_final', 'Kd_final');
...
% total move duration. Distance is kept moderate so the required peak
% velocity stays well under MAX_STEP_RATE, leaving the PID margin to
% actually correct tracking error instead of running flat-out.
rng('shuffle');
...
dwell_time    = 4.0;                                % hold at target after the move, to check final precision
                                                     % (temporarily extended from 1.5s to test whether residual
                                                     % error is still converging or has hit a hard floor)

fprintf('=== Random Trajectory ===\n');
...
% reference was actually in effect at each sample (same technique as
% Hardware_PID_Tuning.m) - the raw 0-360 encoder reading falsely looks
% like a full-circle jump whenever it's near the wrap seam, which is
% exactly where every trajectory here starts.
...
% Your original spec was a PEAK deviation ("+/-0.2 deg at the maximum"),
% not an average - a small persistent oscillation can look fine on the
% mean while still occasionally exceeding the peak requirement, so check
% both instead of only the mean.
...
ylabel('Angle (deg)');
title('Trajectory Tracking (sinusoidal-acceleration profile)');
grid on;
